...
BEGIN
 ...
 END IF;
 UPDATE folioharbor.upload_sessions SET state=p_to,received_bytes=p_received,
  storage_key=COALESCE(p_storage,storage_key),error_code=p_error,updated_at=p_now WHERE upload_id=p_upload;
 RETURN true;
END $$;
...
CREATE TABLE folioharbor.background_jobs (
 ...
 input jsonb NOT NULL CHECK(jsonb_typeof(input)='object' AND input ? 'version'),
