CREATE FUNCTION folioharbor.catalog_item_projection_download_visible(
...
LANGUAGE sql SECURITY DEFINER SET search_path TO '' AS $$
    ...
    FROM folioharbor.catalog_item_projection_visible(
        p_actor, p_library, p_item, p_membership_version
    ) visible
    JOIN folioharbor.libraries library ON library.library_id=p_library
$$;

ALTER FUNCTION folioharbor.catalog_item_projection_download_visible(uuid,uuid,uuid,bigint)
    OWNER TO folioharbor_owner;
REVOKE ALL ON FUNCTION folioharbor.catalog_item_projection_download_visible(uuid,uuid,uuid,bigint)
    FROM PUBLIC,folioharbor_worker;
GRANT EXECUTE ON FUNCTION folioharbor.catalog_item_projection_download_visible(uuid,uuid,uuid,bigint)
    TO folioharbor_api;
...
BEGIN
 ...
 IF candidate.item_id IS NULL THEN
   ...
   RETURN;
 END IF;
 SELECT membership.role_code INTO membership_role
 ...
 permitted := membership_role IN ('owner','editor') OR
     (membership_role='reader' AND candidate.reader_download_enabled);
...
SET schema_version=21,applied_at=clock_timestamp()
WHERE singleton;
