...
semantics. The remaining planned migrations are therefore numbered 0022 through 0024.
...
- Create: `migrations/0014_reader_access_gate.sql`
- Create: `migrations/0015_reader_projection_location.sql`
...
**Task 14 performance and isolation amendment (2026-08-06):** Keep migrations `0012` and `0013`
immutable. Add `0014_reader_access_gate.sql` as an API-only, empty-search-path security-definer
function that performs the lightweight indexed `item.read` gate on every manifest and resource
request. Cache only immutable, package-versioned reader projections after that fresh gate, with
entry and byte bounds, exact eviction accounting, and oversized-entry rejection. Build resource
indexes once so manifest projection is linear and opaque resource resolution is constant-time.
Offload ZIP extraction plus sanitization as one bounded blocking operation, apply semaphore
backpressure, and single-flight identical cold resource reads. Conditional GET accepts strong or
weak matching validators and `*`; resource CSP explicitly includes `frame-ancestors 'self'`, and
untrusted TOC fragments are percent-encoded.

**Task 14 concurrency amendment (2026-08-06):** Resource and projection single-flight work is owned
independently of request waiters, publishes success or failure to every waiter, and removes inflight
state exactly once even when callers cancel. Resource loader creation is bounded by the existing
blocking semaphore. Every projection caller completes its own fresh access gate before consulting
the package-keyed cache or joining one heavy cold load. Additive migration
`0015_reader_projection_location.sql` replaces only the heavy projection function so it selects the
same lexicographically first ready Blob location as the access gate; migrations 0013 and 0014 remain
immutable. The projection byte metric bounds logical string payload; allocator, `Arc`, UUID, and
hash-table structural overhead is additionally capped by the 16-entry cache and EPUB import limits.
...
- Create: `migrations/0016_reading_state.sql`
...
```bash
git add migrations/0016_reading_state.sql crates/domain crates/application crates/postgres crates/http openapi
...
```bash
git add migrations/0020_download_authorization.sql migrations/0021_download_semantics.sql crates/application crates/postgres crates/storage-local crates/http apps/api openapi
...
- Create: `migrations/0022_outbox.sql`
...
```bash
git add migrations/0022_outbox.sql crates/application crates/postgres crates/http apps/worker deploy
...
- Create: `migrations/0023_deletion_and_gc.sql`
...
```bash
git add migrations/0023_deletion_and_gc.sql crates/domain crates/application crates/postgres crates/http apps/worker openapi
...
- Create: `migrations/0024_operations.sql`
...
```bash
git add migrations/0024_operations.sql crates apps deploy docs/operations
