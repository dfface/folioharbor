...
-> anyhow::Result<()> {
    ...
    sqlx::query("INSERT INTO folioharbor.user_accounts(user_id,normalized_email,display_email,status,created_at,verified_at) VALUES($1,'atomic@test.invalid','atomic@test.invalid','verified',$2,$2)")
        .bind(actor.as_uuid()).bind(now).execute(&pools.owner).await?;
    sqlx::query("INSERT INTO folioharbor.libraries(library_id,name,created_at,updated_at) VALUES($1,'Atomic',$2,$2)")
        .bind(library.as_uuid()).bind(now).execute(&pools.owner).await?;
    ...
    let rolled_back: (String, i64) = sqlx::query_as("SELECT state,(SELECT count(*) FROM folioharbor.failed_upload_purges WHERE upload_id=$1) FROM folioharbor.upload_sessions WHERE upload_id=$1")
        .bind(upload.as_uuid()).fetch_one(&pools.owner).await?;
    assert_eq!(rolled_back, ("validating".into(), 0));
    ...
    let committed: (String, i64) = sqlx::query_as("SELECT state,(SELECT count(*) FROM folioharbor.failed_upload_purges WHERE upload_id=$1) FROM folioharbor.upload_sessions WHERE upload_id=$1")
        .bind(upload.as_uuid()).fetch_one(&pools.owner).await?;
    assert_eq!(committed, ("failed".into(), 1));
